...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'receive_po') {
    ...
    try {
        ...
        if (!$po || $po['status'] !== 'approved') {
            throw new DomainException('รับสินค้าได้เฉพาะ PO ที่อนุมัติแล้ว');
        }
        ...
        foreach ((array)($_POST['quantity'] ?? []) as $poItemIdRaw => $quantityRaw) {
            ...
            if ($destination === 'stock') {
                ...
                if ($productId > 0) {
                    ...
                    $productCheck->close();
                    if (!$validProduct) {
                        throw new DomainException('สินค้า Stock ที่เลือกไม่อยู่ในบริษัทปลายทาง');
                    }
                ...
                if ($stockUnit === null) {
                    ...
                    if (!$productRow) {
                        throw new DomainException('ไม่พบหน่วย Stock ของสินค้าที่เลือก');
                    }
        ...
        if ($savedItems === 0) {
            throw new DomainException('กรุณากรอกจำนวนรับอย่างน้อย 1 รายการ');
        }
        $conn->commit();
        stock_flash('success', "บันทึกรับสินค้า {$receiptDocNo} แล้ว");
...
<div class="max-w-7xl mx-auto p-4 md:p-8 space-y-6" data-stock-ui="procurement">
    <div><h1 class="text-2xl font-black text-slate-800 tracking-tight">รับสินค้าเข้าจาก PO</h1><p class="mt-1 text-sm text-slate-500">จัดซื้อเป็นผู้เลือกว่าสินค้าแต่ละรายการเข้า Stock หรือนำไปใช้ทันที</p></div>
    ...
    <form method="get" class="grid grid-cols-1 gap-4 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm md:grid-cols-2">
        <div><div class="text-sm font-semibold text-slate-700">บริษัทปลายทาง</div><div class="mt-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm text-slate-700"><?= $poData ? stock_e($poData['company_name']) : 'ระบบกำหนดจากบริษัทเจ้าของ PO' ?></div></div>
        <label class="text-sm font-semibold text-slate-700">PO ที่อนุมัติแล้วและยังรับไม่ครบ<select name="po_id" required onchange="this.form.submit()" class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"><option value="">เลือก PO</option><?php foreach ($poList as $po): ?><option value="<?= (int)$po['id'] ?>" <?= $selectedPoId === (int)$po['id'] ? 'selected' : '' ?>><?= stock_e($po['doc_no']) ?> · <?= stock_e($po['company_name']) ?> · ค้าง <?= (int)$po['remaining_item_count'] ?> รายการ</option><?php endforeach; ?></select></label>
    ...
    <form method="post" class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        <div class="overflow-x-auto"><table class="min-w-[1180px] w-full text-sm"><thead class="bg-slate-50 text-xs font-bold uppercase tracking-wider text-slate-500"><tr><th class="text-left px-4 py-3">รายการจาก PO</th><th class="text-right px-4 py-3">สั่ง</th><th class="text-right px-4 py-3">รับแล้ว</th><th class="text-left px-4 py-3 w-32">รับครั้งนี้</th><th class="text-left px-4 py-3 w-40">ปลายทาง</th><th class="text-left px-4 py-3 w-[34rem]">สินค้าและหน่วย Stock</th></tr></thead><tbody class="divide-y divide-slate-100">
        <?php foreach ($poItems as $item): $remaining = max(0, (float)$item['item_qty'] - (float)$item['received_quantity']); $savedDestination = $item['saved_destination'] ?: 'stock'; ?>
            <tr class="align-top">
                ...
                <td class="px-4 py-3"><input type="number" min="0" max="<?= $remaining ?>" step="0.01" name="quantity[<?= (int)$item['id'] ?>]" value="0" <?= $remaining <= 0 ? 'disabled' : '' ?> class="w-full rounded-xl border border-slate-200 px-3 py-2 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500 disabled:bg-slate-100"></td>
                ...
                <td class="px-4 py-3">
                    ...
                    <?php else: ?>
                        <div data-stock-product-picker data-item-id="<?= (int)$item['id'] ?>">
                            ...
                            <label class="block text-[11px] font-semibold text-slate-600">ชื่อสินค้า<input name="new_product_name[<?= (int)$item['id'] ?>]" value="<?= stock_e($item['item_desc']) ?>" autocomplete="off" data-product-name class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" placeholder="พิมพ์ชื่อสินค้า"></label>
                            ...
                            <div data-new-product-fields class="mt-2 rounded-lg bg-slate-50 p-3">
                                <label class="block text-[11px] font-semibold text-slate-600">หมวดหมู่สินค้า <span class="text-rose-600">*</span><select name="category_id[<?= (int)$item['id'] ?>]" data-category-select class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs"><?php foreach ($stockCategories as $index => $category): ?><option value="<?= (int)$category['id'] ?>"><?= stock_e($category['name']) ?></option><?php endforeach; ?><option value="0" <?= !$stockCategories ? 'selected' : '' ?>>+ เพิ่มหมวดใหม่</option></select></label>
                            ...
                            <div class="mt-2 rounded-xl border border-indigo-100 bg-indigo-50/60 p-3" data-unit-conversion data-purchase-unit="<?= stock_e($item['item_unit'] ?: 'หน่วย') ?>">
                                <div class="grid grid-cols-2 gap-2">
                                    <label class="block text-[11px] font-semibold text-slate-600">หน่วย Stock/หน่วยเบิก <span class="text-rose-600">*</span><input name="stock_unit[<?= (int)$item['id'] ?>]" data-stock-unit maxlength="50" class="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500" placeholder="เช่น ชิ้น"></label>
        ...
        const renderSuggestions = (resetSelection) => {
            ...
            matches.forEach((product) => {
                ...
                button.addEventListener('click', () => {
                    ...
                    selected.textContent = `เลือกสินค้าเดิม: ${product.name} · ${product.category || 'ไม่ระบุหมวด'}`;
