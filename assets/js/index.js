...
function renderAlert(type) {
  const configs = {
    ...
    duplicate: {
      msg: "ชื่อนี้มีในระบบแล้ว!",
      color: "#f59e0b",
      icon: "bi-exclamation-circle",
    },
    error: {
      msg: "เกิดข้อผิดพลาดบางอย่าง",
      color: "#64748b",
      icon: "bi-x-circle",
    },
  };
...
function createDoc(type) {
  ...
  switch (type) {
    case "quotation":
      targetPage = "quotation_settings.php";
      break;
    case "pr":
      targetPage = "pr_settings.php";
      break;
    case "po":
    ...
    case "invoice":
      ...
      break;
      case "billing":
      targetPage = "billing_settings.php";
      break;
    default:
      targetPage = "other.php";
  }

  // --- Logic พิเศษสำหรับ PR ---
  if (type === "pr") {
    ...
    // ถ้าเลือกมา 1 คน ก็ส่ง ID ไป แต่ถ้าไม่เลือกเลย (length === 0) ก็ไปตัวเปล่าได้เลย
    customerId =
      selectedCheckboxes.length === 1 ? selectedCheckboxes.val() : "";

    // ส่งไปที่หน้า PR (ถ้า customerId ว่าง หน้าโน้นจะรู้เองว่าให้ใช้ข้อมูล User Login)
    window.location.href = `${targetPage}${customerId ? "?customer_id=" + customerId : ""}`;
    return;
  }
  ...
  customerId = selectedCheckboxes.val();
...
function fillMockCustomer() {
  ...
  setTimeout(
    () => $("#customerFormSection").removeClass("ring-2 ring-amber-400"),
    500,
  );
}
