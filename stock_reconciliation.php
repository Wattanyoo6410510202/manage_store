...
if (!in_array($stock_actor['role'], ['procure', 'gmhr', 'admin'], true)) {
    ...
    die('คุณไม่มีสิทธิ์เข้าถึงการกระทบยอดสินค้า');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'create') {
            ...
            $productStmt->close();
            if (!$products) {
                throw new DomainException('บริษัทนี้ยังไม่มีสินค้าให้กระทบยอด');
            }
            ...
            foreach ($products as $product) {
                if (!array_key_exists($product['id'], $counts) || !is_numeric($counts[$product['id']]) || (float)$counts[$product['id']] < 0) {
                    throw new DomainException('กรุณากรอกยอดนับจริงของสินค้าทุกรายการ');
                }
