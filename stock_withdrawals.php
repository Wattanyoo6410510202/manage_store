...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'create_withdrawal') {
    try {
        ...
        foreach ((array)($_POST['quantity'] ?? []) as $productIdRaw => $quantityRaw) {
            ...
            $check->close();
            if (!$valid) {
                throw new DomainException('พบพัสดุที่ไม่อยู่ในบริษัทของใบเบิก');
            }
...
foreach ($companies as $company) {
    if ((int)$company['id'] === $requestSupId) {
        $requestCompanyName = supplier_display_name($company['company_name']);
        break;
    }
}
...
<div class="max-w-7xl mx-auto p-4 md:p-8 space-y-6" data-stock-ui="procurement">
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><h1 class="text-2xl font-black text-slate-800 tracking-tight">ใบเบิกพัสดุ</h1><p class="mt-1 text-sm text-slate-500">จำนวนที่ขอเบิกต้องเป็นจำนวนเต็มและไม่เกินยอดคงเหลือใน Stock ปัจจุบัน</p></div><?php if (stock_can_manage($stock_actor['role'])): ?><form method="get" class="w-full md:w-80"><label class="mb-1 block text-sm font-semibold text-slate-700">บริษัท/หน่วยงาน</label><select name="sup_id" onchange="this.form.submit()" class="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm shadow-sm outline-none transition-all focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500"><option value="0">ทุกบริษัท</option><?php foreach ($companies as $company): ?><option value="<?= (int)$company['id'] ?>" <?= (int)($_GET['sup_id'] ?? 0) === (int)$company['id'] ? 'selected' : '' ?>><?= stock_e(supplier_display_name($company['company_name'])) ?></option><?php endforeach; ?></select></form><?php endif; ?></div>
    ...
    <form method="post" data-stock-withdrawal-cart>
        <details class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" open>
            <summary class="flex cursor-pointer list-none justify-between p-5 font-bold text-slate-800"><span><i class="fas fa-shopping-basket mr-2 text-indigo-600"></i>เลือกพัสดุเพื่อสร้างใบเบิก</span><i class="fas fa-chevron-down text-xs text-slate-400 transition-transform group-open:rotate-180"></i></summary>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2">
                                <div class="flex min-w-0 flex-1 items-center overflow-hidden rounded-xl border border-slate-200 bg-white">
                                    ...
                                    <input data-stock-product-quantity type="number" min="1" max="<?= $availableQuantity ?>" step="1" inputmode="numeric" value="<?= $availableQuantity > 0 ? 1 : 0 ?>" <?= $availableQuantity <= 0 ? 'disabled' : '' ?> class="h-11 min-w-0 flex-1 border-x border-slate-200 text-center font-bold text-slate-900 outline-none focus:bg-indigo-50 disabled:bg-slate-100 disabled:text-slate-400">
                                ...
                                <button type="button" data-stock-add-to-cart <?= $availableQuantity <= 0 ? 'disabled' : '' ?> class="min-w-[8.5rem] rounded-xl bg-indigo-600 px-3 py-2.5 text-sm font-bold text-white transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-slate-300"><i class="fas fa-cart-plus mr-1.5"></i><?= $availableQuantity > 0 ? 'เพิ่มลงตะกร้า' : 'พัสดุหมด' ?></button>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
                    <?php if ($products): ?><div data-stock-category-empty class="hidden border-t border-slate-200 px-4 py-10 text-center text-sm text-slate-500"><i class="fas fa-search mb-3 block text-2xl text-slate-300"></i>ไม่พบพัสดุที่ตรงกับคำค้นหาและหมวดหมู่</div><?php else: ?><div class="px-4 py-12 text-center text-sm text-slate-500"><i class="fas fa-box-open mb-3 block text-2xl text-slate-300"></i>บริษัทนี้ยังไม่มีพัสดุใน Stock</div><?php endif; ?>
                </div>
            </div>
            <div class="border-t border-slate-200 bg-slate-50/70 px-5 py-4 text-xs text-slate-500"><i class="fas fa-circle-info mr-1 text-indigo-500"></i>เพิ่มพัสดุได้หลายรายการ และจำนวนรวมต้องไม่เกินยอดคงเหลือ</div>
            </div>
        </details>
        ...
        <aside id="stock-withdrawal-cart-panel" data-stock-cart-panel aria-hidden="true" aria-labelledby="stock-withdrawal-cart-title" class="fixed inset-x-0 bottom-0 z-50 flex max-h-[88vh] translate-y-full flex-col rounded-t-3xl border border-slate-200 bg-slate-50 shadow-2xl transition-transform duration-200 md:inset-y-0 md:left-auto md:right-0 md:max-h-none md:w-[28rem] md:translate-y-0 md:translate-x-full md:rounded-none md:rounded-l-3xl">
            <div class="flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-5 py-4 md:rounded-tl-3xl">
                <div><h2 id="stock-withdrawal-cart-title" class="text-lg font-black text-slate-900"><i class="fas fa-shopping-cart mr-2 text-indigo-600"></i>ตะกร้าใบเบิก</h2><p data-stock-cart-summary class="mt-1 text-sm text-slate-500">ยังไม่มีพัสดุ</p></div>
            ...
            <div class="flex-1 space-y-3 overflow-y-auto p-5">
                <div data-stock-cart-empty class="rounded-2xl border-2 border-dashed border-slate-200 bg-white px-5 py-10 text-center text-sm text-slate-500"><i class="fas fa-basket-shopping mb-3 block text-3xl text-slate-300"></i>ยังไม่มีพัสดุในตะกร้า<br><span class="text-xs">เลือกจำนวนแล้วกด “เพิ่มลงตะกร้า”</span></div>
