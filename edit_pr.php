            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-white p-5 rounded-3xl border border-slate-200">
                        ...
                        <select name="supplier_id" id="supplier_select" onchange="updateSupplierInfo()" class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm focus:border-indigo-500 focus:bg-white outline-none font-bold text-slate-700 mb-5 transition-all cursor-pointer">
                            <?php foreach ($suppliers as $sup): ?>
                                <option value="<?= $sup['id'] ?>" 
                                    data-info='<?= json_encode($sup, ENT_QUOTES) ?>'
                        ...
                        <div class="p-5 bg-gradient-to-br from-indigo-50/50 to-slate-50 rounded-2xl border border-indigo-100/50 space-y-4">
                            <div>
                                <div id="comp_name" class="text-base font-black text-slate-800 leading-tight">กำลังโหลดข้อมูล...</div>
                    ...
                    <div class="bg-slate-900 rounded-3xl overflow-hidden border border-slate-800">
                        ...
                        <div class="p-6 space-y-4">
                            <div>
                                <div class="text-white font-black text-lg"><?= htmlspecialchars($customer['customer_name'] ?? 'ไม่พบข้อมูล') ?></div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white p-6 rounded-3xl border border-slate-200 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase block mb-1">เลขที่เอกสาร</label>
                            <input type="text" value="<?= $pr_data['doc_no'] ?>" class="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-xl text-sm font-bold outline-none text-slate-500" readonly>
                        </div>
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase block mb-1">วันที่ต้องการสินค้า</label>
                        ...
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase block mb-1">อ้างอิงเอกสาร (Ref.)</label>
                        ...
                        <div>
                            <label class="text-[10px] font-black text-slate-400 uppercase block mb-1">การชำระเงิน</label>
                        ...
                        <div class="col-span-1">
                            <label class="text-[10px] font-black text-slate-400 uppercase block mb-1">ผู้ต้องการ / แผนก</label>
                        ...
                        <div class="col-span-1">
                            <label class="text-[10px] font-black text-slate-400 uppercase block mb-1">เบอร์โทรผู้ติดต่อ</label>
                    ...
                    <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden">
                        <div class="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/30">
                            ...
                            <button type="button" onclick="addItemRow()" class="px-4 py-2 bg-indigo-600 text-white text-[11px] font-black rounded-xl hover:bg-indigo-700 transition-all ">
                            ...
                            <table class="w-full text-left border-collapse" id="itemsTable">
                                ...
                                <tbody class="divide-y divide-slate-50">
                                    ...
                                    <tr class="item-row group">
                                        <td class="px-6 py-4 text-center text-xs font-bold text-slate-300"><?= $count++ ?></td>
                                        <td class="px-2 py-4">
                                            <textarea name="item_desc[]" rows="1" oninput="autoResize(this)" class="w-full bg-transparent border-none focus:ring-0 outline-none text-sm text-slate-700 font-bold resize-none block overflow-hidden"><?= htmlspecialchars($item['item_desc']) ?></textarea>
                                        </td>
                                        <td class="px-2 py-4">
                                            <input type="number" name="item_qty[]" value="<?= $item['item_qty'] ?>" min="1" oninput="calculateTotal()" class="w-full bg-slate-50 border-none rounded-lg px-2 py-2 text-center text-sm font-black text-indigo-600 focus:bg-indigo-50">
                                        </td>
                                        <td class="px-2 py-4">
                                            <input type="text" name="item_unit[]" value="<?= htmlspecialchars($item['item_unit']) ?>" class="w-full bg-transparent border-b border-slate-100 text-center text-xs font-bold outline-none focus:border-indigo-400">
                                        </td>
                                        <td class="px-2 py-4">
                                            <input type="number" name="item_price[]" value="<?= $item['item_price'] ?>" step="0.01" oninput="calculateTotal()" class="w-full bg-transparent border-none text-right text-sm font-mono font-black focus:ring-0">
                                        </td>
                        ...
                        <div class="p-6 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-start gap-6">
                            <div class="w-full md:flex-grow">
                                ...
                                <textarea name="notes" rows="3" class="w-full p-3 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500 resize-none"><?= htmlspecialchars($pr_data['notes']) ?></textarea>
                            </div>
                            <div class="w-full md:w-72 space-y-2">
                                ...
                                <button class="w-full mt-4 py-3 bg-indigo-600 text-white font-black rounded-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-2">
...
<script>
    // คงฟังก์ชันเดิมของคุณไว้ทั้งหมด แต่ปรับปรุงให้รองรับข้อมูลที่โหลดมาใหม่
    function addItemRow() {
        ...
        <tr class="item-row group border-t border-slate-100">
            ...
            <td class="px-4 py-3">
                <textarea name="item_desc[]" placeholder="ระบุรายละเอียด..." rows="1" oninput="autoResize(this)" class="w-full bg-transparent border-none focus:ring-0 outline-none text-sm text-slate-700 font-medium resize-none block overflow-hidden"></textarea>
            </td>
            <td class="px-4 py-3">
            ...
            <td class="px-4 py-3">
                <input type="text" name="item_unit[]" placeholder="เช่น ตัว, ชุด" class="w-full bg-slate-50 border-none rounded-md px-2 py-1 text-center text-sm font-bold focus:bg-indigo-50">
            </td>
            <td class="px-4 py-3">
