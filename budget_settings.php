...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'save') {
        ...
        if (empty($id)) {
            // โหมดเพิ่มใหม่
            $sql = "INSERT INTO budget_types (sup_id, name, is_active) VALUES ($sup_id, '$name', 1)";
        } else {
            // โหมดแก้ไข
            $sql = "UPDATE budget_types SET sup_id = $sup_id, name = '$name' WHERE id = " . intval($id);
        }
...
function fetchBudget() {
    $.get('?action=fetch', function(data) {
        ...
        if(!data || data.length === 0) {
            html = '<tr><td colspan="3" class="p-12 text-center text-slate-400 font-medium">ไม่พบข้อมูลประเภทงบประมาณ</td></tr>';
        } else {
            data.forEach(item => {
                html += `
