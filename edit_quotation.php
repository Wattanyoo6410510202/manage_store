                ...
                <div class="lg:col-span-1 space-y-6">
                    <div class="bg-white p-5 rounded-3xl border border-slate-200 ">
                        ...
                        <select name="supplier_id" id="supplier_select" onchange="updateSupplierInfo()"
                            ...
                            <option value="0" <?= (empty($data['supplier_id'])) ? 'selected' : '' ?>>--- เลือกผู้ขาย ---
                            </option>

                            <?php if (!empty($suppliers)): ?>
                                <?php foreach ($suppliers as $sup): ?>
                                    <option value="<?= $sup['id'] ?>" data-info='<?= json_encode($sup, ENT_QUOTES) ?>'
                                        <?= (isset($data['supplier_id']) && $sup['id'] == $data['supplier_id']) ? 'selected' : '' ?>>
                        ...
                        <div
                            ...
                            <div>
                                <div id="comp_name" class="text-base font-black text-slate-800 leading-tight">
                                    <?= (empty($data['supplier_id'])) ? 'ยังไม่ได้เลือกผู้ขาย' : 'กำลังโหลด...' ?>
                                </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                            ...
                            <div>
                                ...
                                <select name="vat_percent" id="vat_percent" onchange="calculateTotal()"
                                    class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold outline-none focus:border-indigo-500">
                                    <option value="7" <?= ($data['vat_percent'] == 7) ? 'selected' : '' ?>>7%</option>
                                    <option value="0" <?= ($data['vat_percent'] == 0) ? 'selected' : '' ?>>0% (ไม่มีภาษี)
                                    </option>
                                    <option value="10" <?= ($data['vat_percent'] == 10) ? 'selected' : '' ?>>10%</option>
                                    <option value="3" <?= ($data['vat_percent'] == 3) ? 'selected' : '' ?>>3%</option>
                                    <option value="5" <?= ($data['vat_percent'] == 5) ? 'selected' : '' ?>>5%</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="wht_percent" id="wht_percent" onchange="calculateTotal()"
                                    class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold outline-none focus:border-indigo-500">
                                    <option value="0" <?= ($data['wht_percent'] == 0) ? 'selected' : '' ?>>0%</option>
                                    <option value="1" <?= ($data['wht_percent'] == 1) ? 'selected' : '' ?>>1%</option>
                                    <option value="3" <?= ($data['wht_percent'] == 3) ? 'selected' : '' ?>>3%</option>
                                    <option value="5" <?= ($data['wht_percent'] == 5) ? 'selected' : '' ?>>5%</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="bg-slate-900 rounded-xl overflow-hidden border border-slate-800">
                        <div
                            ...
                            <h3
                                class="text-indigo-400 text-[11px] font-black flex items-center gap-2 uppercase tracking-wider">
                                <i class="fas fa-user-tie"></i> Client / ผู้ซื้อ
                            </h3>
                            <?php if ($num_rows > 20): ?>
                                <span
                                    class="text-[10px] bg-red-500/20 text-red-400 px-2 py-0.5 rounded-full animate-pulse italic">!
                                    Items exceed 20 rows</span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="p-4 flex flex-col gap-3">
                            <div class="flex justify-between items-end border-b border-slate-800 pb-2">
                                <div>
                                    <label class="text-[9px] uppercase font-bold text-slate-500 block">Customer
                                        Name</label>
                                ...
                                <div class="text-right">
                                    <label class="text-[9px] uppercase font-bold text-slate-500 block">Tax ID</label>
                                    <div class="text-[11px] text-indigo-300 font-mono"><?= $data['cust_tax'] ?: '-' ?>
                                    </div>
                                </div>
                            </div>

                            <div class="grid grid-cols-3 gap-4">
                                <div class="col-span-2 border-r border-slate-800 pr-4">
                                    <label class="text-[9px] uppercase font-bold text-slate-50ห0 block">Address</label>
                                    <div class="text-[11px] text-slate-400 line-clamp-2 leading-tight">
                                        <?= htmlspecialchars($data['cust_address']) ?>
                                    </div>
                                </div>
                                <div>
                                    <label class="text-[9px] uppercase font-bold text-slate-500 block">Contact</label>
                                    <div class="text-[11px] text-white font-bold truncate">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden mt-4">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse" id="itemsTable">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php
                            $count = 1;
                            mysqli_data_seek($res_items, 0); // รีเซ็ต pointer
                            while ($item = mysqli_fetch_assoc($res_items)):
                                ?>
                                <tr class="item-row group">
                                    <td class="px-4 py-3 text-center text-xs font-bold text-slate-400 row-number">
                                        <?= $count++ ?>
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        <input type="number" name="item_discount[]"
                                            value="<?= $item['item_discount'] ?? 0 ?>" step="0.01"
                ...
                <div
                    ...
                    <div class="w-full md:w-72 space-y-2">
                        ...
                        <div class="flex justify-between text-sm font-bold text-slate-500">
                            <span>ภาษี (<span
                                    id="vat_percent_label"><?= number_format($data['vat_percent'] ?? 7, 0) ?></span>%)</span>
                            <span id="vat_display"><?= number_format($data['vat'], 2) ?></span>
                        </div>

                        <div class="flex justify-between text-sm font-bold text-slate-500">
                            <span>หัก ณ ที่จ่าย (<span
                                    id="wht_percent_label"><?= number_format($data['wht_percent'] ?? 0, 0) ?></span>%)</span>
                            <span id="wht_display"><?= number_format($data['wht_amount'], 2) ?></span>
                        </div>
...
<script>
    ...
    function addItemRow() {
        ...
        <tr class="item-row group border-t border-slate-100">
            ...
            <td class="px-4 py-3">
                <input type="number" name="item_discount[]" value="0.00" step="0.01" oninput="calculateTotal()" class="w-full bg-amber-50 border-none rounded-md px-2 py-1 text-right text-sm font-mono font-bold text-amber-600 focus:bg-amber-100 outline-none">
            </td>
    ...
    // 2. ฟังก์ชันลบแถวและเรียงเลขใหม่
    function removeRow(btn) {
        const tbody = document.querySelector('#itemsTable tbody');
        if (tbody.rows.length > 1) {
            btn.closest('tr').remove();
            calculateTotal();
            // เรียงเลขแถวใหม่ให้ถูกต้องเสมอ
    ...
    // 3. ฟังก์ชันคำนวณยอดรวม (หักส่วนลดก่อนคิด VAT)
    function calculateTotal() {
        let subtotal = 0;

        // 1. ดึงค่า VAT % และ WHT % จาก Select ที่เราทำไว้
        ...
        // 3. คำนวณภาษีมูลค่าเพิ่ม (VAT)
        const vatAmount = subtotal * (vatPercent / 100);

        // 4. คำนวณหัก ณ ที่จ่าย (WHT) - คิดจากยอดก่อน VAT (Subtotal)
        const whtAmount = subtotal * (whtPercent / 100);

        // 5. ยอดสุทธิที่ต้องชำระ (รวม VAT แต่หัก WHT ออก)
        ...
        // หัก ณ ที่จ่าย (ถ้ามี Element แสดงผล)
        if (document.getElementById('wht_display')) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // สั่งให้ Textarea ทุกตัวขยายความสูงตามเนื้อหาที่มีอยู่เดิม
